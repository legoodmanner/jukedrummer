...
class DrumAwareBeatTracker(nn.Module):
    def __init__(
        ...
        self.fuserBeat = RNNDownBeatProc(9, fuser_blstm_hidden_size, fuser_nb_layers)
        if FuserBeat_chkpnt:
    ...
    def forward(self, x):
        ...
        if self.fixed_drum:
            self.drumBeat.eval()
            with torch.no_grad():
                beat_drum = self.drumBeat(x_drum.detach())
        else:
            beat_drum = self.drumBeat(x_drum.detach())
        ...
        if self.fixed_nodrum:
            ...
            with torch.no_grad():
                beat_nodrum = self.nodrumBeat(x_nodrum.detach())
        else:
            beat_nodrum = self.nodrumBeat(x_nodrum.detach())
        ...
        if self.fixed_mix:
            self.mixBeat.eval()
            with torch.no_grad():
                beat_mix = self.mixBeat(x)
        else:
            beat_mix = self.mixBeat(x)
        combined_pred = torch.cat([beat_mix.detach(), beat_nodrum.detach(), beat_drum.detach()], dim = -1)
