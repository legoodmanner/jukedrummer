...
## Revised from https://github.com/openai/jukebox/blob/master/jukebox/prior/autoregressive.py
...
class ConditionalAutoregressive2D(nn.Module):
    def __init__(self, input_shape, bins,
                 ...
                 encoder_dims=0, only_encode=False, merged_decoder=False, prime_len=None, pos_emb=None):
        super().__init__()
        self.input_shape = input_shape
        self.input_dims = input_dims = np.prod(input_shape)
        ...
        self.x_data_dropout = nn.Dropout2d(emb_dropout)
        self.y_cond = y_cond
        self.x_cond = x_cond
        if not y_cond:
            self.start_token = nn.Parameter(get_normal(1, width, std=0.01 * init_scale))

        self.pos_emb = pos_emb or PositionEmbedding(input_shape=input_shape, width=width, init_scale=init_scale)
